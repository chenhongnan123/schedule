...
async function maintenanceTaskJob(plan) {
  try {
    let payload = {
      ...
      id: `${plan[maintenanceplanTags.PLANID_TAG]}-${new Date().getTime()}`,
    ...
    if (operators.status === HTTP_STATUS_CODE.SUCCESS) {
        ...
        if (response.status === HTTP_STATUS_CODE.SUCCESS) {
          if (response.data.results.length > 0) {
            ...
            if (details.length) {
              ...
              createTaskOperator(plan, task, details);
            }
            // Create Replacement
      ...
      checkSessionExpired(operators.data);
      log.error(
        `Error in writing Maintenance_Task in ShopWorx ${JSON.stringify(
          operators.data
        )}`
...
async function createTaskOperator(plan, task, details) {
  try {
    if (details.length > 0) {
