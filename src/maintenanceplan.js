...
function maintenanceplan(config, utility, tags, emitter) {
  ...
  return {
    ...
    initEmitter() {
      emitter.on('maintenanceplan', (data) => {
        if (data[maintenanceplanTags.STATUS_TAG] == MaintenancePlanStatusValue.ENABLE) {
          log.info(`Get New MaintenancePlan: ${JSON.stringify(data)}`);
          ...
          log.info(`All maintenanceplan till now  ${JSON.stringify(this.maintenanceplanList)}`);
        } else {
          // remove  schedule
    ...
    async maintenanceTaskJob(plan, count) {
      try {
        let payload = {
          ...
          [maintenancetaskTags.DATE_TAG]: Number(format(new Date(), "yyyyMMdd")),
          assetid: plan.assetid,
          [maintenancetaskTags.TYPE_TAG]: 'CBM',
        };
        const operatorquery = `query=machineid=="${plan[maintenanceplanTags.MACHINEID_TAG]}"`;
        const operators = await elementService.getElementRecords(
          elements.operatorbindmachine || 'operatorbindmachine',
          operatorquery
        );
        if (operators.status === utility.HTTP_STATUS_CODE.SUCCESS) {
          const details = operators.data.results;
          if (details.length > 0) {
            payload[maintenancetaskTags.STATUS_TAG] = 'assigned'
          }
            ...
            if (response.status === utility.HTTP_STATUS_CODE.SUCCESS) {
              if (response.data.results.length > 0) {
                ...
                if (details.length) {
                  ...
                  this.createTaskOperator(plan, task, details);
                }
          ...
          utility.checkSessionExpired(operators.data);
          log.error(
            `Error in writing Maintenance_Task in ShopWorx ${JSON.stringify(
              operators.data
            )}`
    ...
    async createTaskOperator(plan, task, details) {
      try {
        if (details.length > 0) {
